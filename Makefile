...
docker-build: ## 构建 Docker 镜像
	@docker build -t starcat-sharing-api:latest .
	@echo "✓ Docker 镜像构建完成: starcat-sharing-api:latest"

.PHONY: docker-run
docker-run: ## 运行 Docker 容器
	@docker run --rm -p 5002:5002 starcat-sharing-api:latest

.PHONY: docker-clean
docker-clean: ## 清理 Docker 镜像
	@docker rmi starcat-sharing-api:latest 2>/dev/null || true
	@echo "✓ Docker 镜像已清理"
