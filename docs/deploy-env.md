...
`BASE_URL` 必须保持 `https://starcat.ink`。Universal Links、AASA 与客户端分享 URL
都以无 `www` 主域为唯一协议；如果需要兼容 `www`，应由 Nginx 重定向到主域，不能
只修改 sharing-api 的 secret。
...
| `GITHUB_TOKENS` | GitHub PAT 池（逗号分隔） | **fly secrets** | `fly secrets set GITHUB_TOKENS=...` |
